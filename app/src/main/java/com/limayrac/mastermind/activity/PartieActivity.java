...
public class PartieActivity extends Activity implements View.OnClickListener {
    TableRow saisieUtilisateur;
...
//        int width = displayMetrics.widthPixels;
        ...
        // On génère le code couleur secret
        original = CodeCouleur.generateRandom(niveauDifficulte.getCouleurSaisie(), niveauDifficulte.getCouleurPropose());
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.validate_button) {
            if (saisie_courante == niveauDifficulte.getCouleurSaisie()) {
                ...
                CodeReponse codeReponse = original.compare(codeCouleur);
                ...
                if (codeReponse.isOk()) {
                    ...
                    // On montre le dialogue APRES avoir sauvegardé en BDD
                    showDialogWin(adapter.getCount());
                }

                if (adapter.getCount() == niveauDifficulte.getNombreEssai()) {
                    CodeCouleur toSave = codeCouleurDao.save(codeReponse.getOriginal());
                    Score score = new Score();
                    score.setDate(LocalDate.now());
                    score.setNbTentative(adapter.getCount());
...
//            }
    ...
    public void showDialogLose() {
        ...
        builder.setMessage(getString(R.string.lose_content));
